...
plot(g,layout=layout_nicely, 
     ...
     vertex.lable.cex=0.7,
     VERTEX.size=EDGE$count*10,margin=-0.1,edge.arrow.size=0.3,
     edge.arrow.width=0.5, edge.color="black",vertex.label.color="black")
```
## Part III
...
plot(g1, layout=layout.fruchterman.reingold, 
     vertex.size=6, 
     #degree(g)*0.9, 
     vertex.label.cex=0.6,vertex.label.color="black",vertex.color="blue" )
```
...
# Accourding to the data we have, Yifei Zhang is the most central person in the network.
```
...
C1<-read.csv("hudk4050-classes.csv",stringsAsFactors = FALSE,header = TRUE)
C1<-C1[-c(1,2),]
names(C1)<-c("First name","Last name","1","2","3","4","5","6","which is most interesting")
C2 <- select(C1,"First name","Last name","1","2","3","4","5","6")
C3 <- unite(C2,"name","First name","Last name",sep = "")
C3$name <-str_replace(C3$name,"`","")
C3$name <- str_to_title(C3$name)
C3 <- C3 %>% mutate_at(2:7,list(toupper))
C3 <- C3 %>% mutate_at(2:7, str_replace_all, " ","")

A<-C1$`which is most interesting`
C1$`which is most interesting`<-ifelse(is.na(A)==TRUE,as.factor(as.character(A)),as.factor(A))
C3$Interest<-C1$`which is most interesting`
plot(g,layout = layout.fruchterman.reingold,vertex.size=7,#degree(g)*0.7,
     vertex.label.cex=0.3,
     vertex.label.color="gray", 
     vertex.color=C3$Interest)
# The interest "Social Network Analysis" (Yellow) corresponds to the clusters in the network. 
```
